...
def translateToLLVM(s, config, settings, args=None, vocab=None, check_success=False, var_count=None):
	separator = ' ; ' if settings.getboolean('LLVM', 'AppendSemicolon') else ' '
	with open('tmp' + str(os.getpid()) + '.c', 'w') as f:
		f.write('int ' + ','.join(map(lambda i: 'X' + str(i), range(
			var_count if var_count else config.getint('Var', 'NumVars')))) + ','.join(
			[''] + map(lambda i: 'N' + str(i), range(config.getint('Number', 'NumbersPerStatement')))) + ';\n')
		f.write('void f() {\n')
		for x in s:
			f.write(str(x) + '\n')
		f.write('}\n')
