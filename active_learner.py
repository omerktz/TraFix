...
class ActiveLearner:
	def __init__(self, input, output_dir, experiment=False, codenator_config='configs/codenator.config',
				 dynmt_config='configs/dynmt.config', num_translations=5, success_percentage=0.99, validation_size=1000,
				 train_size_initial=10000, train_size_increment=10000, initial_model=None):
		# store parameters
		self.input = input
	...
	def update_datasets(self, i):
		...
		while True:
			...
			vocabs_utils.generate_vocabs(os.path.join(self.datasets_path, 'validate%d' % i))
...
if __name__ == "__main__":
	...
	ActiveLearner(input=args.input, output_dir=args.output, experiment=args.experiment,
				  codenator_config=args.codenator_config, dynmt_config=args.dynmt_config,
