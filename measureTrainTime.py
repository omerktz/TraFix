...
def run((nums, length)):
	...
	os.system(
		'(time python api_dynmt.py datasets/train_{0}_{1} datasets/validate_{0}_{1} datasets/test_{0}_{1} -m models/model_{0}_{1} -po -c configs/dynmt.config --train --cleanup) &> outputs/dynmt_train_{0}_{1}.out'.format(
			nums, length))
	train_end = time.time()
	...
	os.system(
		'(time python api_dynmt.py datasets/train_{0}_{1} datasets/validate_{0}_{1} datasets/test_{0}_{1} -m models/model_{0}_{1} -po -c configs/dynmt.config --translate --cleanup) &> outputs/dynmt_translate_{0}_{1}.out'.format(
			nums, length))
