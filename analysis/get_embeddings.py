...
if len(sys.argv) == 4:
  ...
  with open('embeddings.html', 'w') as fhtml:
    ...
    fhtml.write('var types = [' + ','.join(map(lambda x: '"' + getType(x).name + '"', vocab)) + '];\n')
    fhtml.write('var title = "' + sys.argv[1] + ' embeddings";\n')
    fhtml.write('var matrix = [')
    for i in range(len(vocab)):
      if i > 0:
        fhtml.write(',')
      fhtml.write('[' + ','.join(map(lambda j: str(embedding_distance(embeddings[i], embeddings[j])), range(len(vocab)))) + ']')
    fhtml.write('];\n')
