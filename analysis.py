...
def get_datasets_size(folder, prefix, suffix):
	iterations = set()
	for f in os.listdir(os.path.join(folder, 'datasets')):
		if f.startswith(prefix) and f.endswith(suffix):
			iterations.add(int(f[len(prefix):-len(suffix)]))
	result = {}
	for i in iterations:
...
def get_timings(log, marker, until=1):
	def get_time(line):
		index = line.find(' ')
...
def num_of_iterations(keys, logs, outputs, out):
	def logic(log, output):
		with open(log, 'r') as f:
			lines = filter(lambda l: len(l.strip()) > 0, f.readlines())
			last_line = lines[-1]
			last_line = last_line[last_line.find('%) in ') + 6:]
			last_line = last_line[:last_line.find(' ')]
			return int(last_line)

	data = analyze_all_results(logic, keys, logs, outputs)
	out.writerow(['Number of Iterations'])
	out.writerow(['', 'Raw:'] + data)
	out.writerow(['', 'Min:', min(data), '', 'Max:', max(data), '', 'Average:', sum(data) / len(data)])
	out.writerow([])
...
def train_time(keys, logs, outputs, out):
	def logic(log, output):
		return get_timings(log, 'Training model ')
...
def translate_time(keys, logs, outputs, out):
	def logic(log, output):
		return get_timings(log, 'Translating dataset ')
...
def evaluate_time(keys, logs, outputs, out):
	def logic(log, output):
		return get_timings(log, 'Evaluating latest results ')
...
def iteration_time(keys, logs, outputs, out):
	def logic(log, output):
		return get_timings(log, 'Training model ', until=4)
...
def train_epochs(keys, logs, outputs, out):
	def logic_last(log, output):
		iterations = set()
		for f in os.listdir(os.path.join(output, 'outputs')):
			if f.startswith('train'):
				iterations.add(int(f[len('train'):]))
		result = {}
		for i in iterations:
...
analysis_funcs = [num_of_iterations, remaining_test_size, train_size, train_time, translate_time, evaluate_time,
				  iteration_time, train_epochs]


def analyze_results(input, output, funcs=analysis_funcs):
	logs = {}
	outputs = {}
	...
	with open(output, 'w') as fout:
		...
		csvout.writerow([])
		for f in funcs:
			f(sorted(keys), logs, outputs, csvout)
...
if __name__ == "__main__":
	...
	analyze_results(input=args.input, output=args.output)
