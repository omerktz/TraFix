...
def train(hparams, scope=None, target_session=""):
  ...
  dev_src_file = "%s.%s" % (hparams.dev_prefix, hparams.src)
  dev_tgt_file = "%s.%s" % (hparams.dev_prefix, hparams.tgt)
  sample_src_data = inference.load_data(dev_src_file, split_digits=hparams.split_numbers_in, filter_tokens=[';', ','])
  sample_tgt_data = inference.load_data(dev_tgt_file, split_digits=hparams.split_numbers_out)
  ...
  while global_step < num_train_steps and patience < hparams.patience:
    ...
    if global_step - last_eval_step >= steps_per_valid:
      ...
      run_sample_decode(infer_model, infer_sess,
                        ...
                        sample_tgt_data)
      dev_ppl , tst_ppl = run_internal_eval(
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
  ...
  utils.print_out(b"    tfnmt: " + translation)
