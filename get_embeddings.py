...
if len(sys.argv) == 4:
  ...
  with open(sys.argv[2]+'.csv','w') as f:
    ...
    for i in range(len(vocab)):
      f.write(vocab[i].repalce(',','","')+','+embeddings[i].repalce(',','","')+',,'+embeddings[i].repalce(',','","').replace(' ',',')+'\n')
  os.remove(sys.argv[2])
  with open(sys.argv[2]+'.dist.csv', 'w') as f:
    f.write(','+','.join(map(lambda i:vocab[i].repalce(',','","'), range(len(vocab))))+'\n')
    for i in range(len(embeddings)):
      embeddings[i] = map(lambda y: float(y), filter(lambda x: len(x)>0, embeddings[i].split(' ')))
    for i in range(len(vocab)):
      f.write(vocab[i].repalce(',','","')+','+','.join(map(lambda j:str(embedding_distance(embeddings[j], embeddings[i])), range(len(vocab))))+'\n')