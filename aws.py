...
class AWShandler:
    ...
    def create_instance(self):
        ...
        instance.create_tags(Tags=[
            {'Key': 'Name', 'Value': self._instance_name.format(getpass.getuser(), self._compiler[:-7], self._index)}])
    ...
    def get_client(self):
        dns = self._ec2client.describe_instances(InstanceIds=[self._instance.id])['Reservations'][0]['Instances'][0][
            'PublicDnsName']
    ...
    def exec_instance(self):
        ...
        self.exec_command(
            'cd {0}; {3}./runExperiment_forConfigs.sh output{1} log{1} {2}'.format(self._main_dir, self._index, self._compiler,
                                                                        activate_gpu_workspace if self._gpu else ''))
    ...
    def download_from_instance(self):
        ...
        sftp.get('output{0}.tar.gz'.format(self._index),
                 os.path.join(self._output, 'output{0}.tar.gz'.format(self._index)))
...
def instance_wrapper((args, i)):
    ...
    hide_logs()
    if args.gpu:
        AWShandler(args.compiler, args.output, i, image=args.gpu_image, username=args.username, key=args.key,
                   retries=args.retries,
                   instance_type=args.gpu_type, security_group=args.security, termination_protection=args.protection,
                   instance_name=args.gpu_naming, main_dir=args.directory, branch=args.gpu_branch,
                   config_dir=args.configs, gpu=True).launch_instance()
    else:
        AWShandler(args.compiler, args.output, i, image=args.image, username=args.username, key=args.key,
                   ...
                   instance_name=args.naming, main_dir=args.directory, branch=args.branch, config_dir=args.configs,
                   gpu=False).launch_instance()
